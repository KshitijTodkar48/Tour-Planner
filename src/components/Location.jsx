...
const Location = (props) => {
  ...
  const filteredHistoricalArray = HistoricalArray.filter((place) => {
    return place.days.includes(props.id1) ;
  });

  const filteredReligiousArray = ReligiousArray.filter((place) => {
    return place.days.includes(props.id1) ;
  });

  const filteredNaturalArray = NaturalArray.filter((place) => {
    return place.days.includes(props.id1) ;
  });
  ...
  return (
    <div className="location">
      {props.id2 !== "" ? (
        <>
          {props.id2 === "Historical/Heritage" ? (
            <>
              { filteredHistoricalArray.map((place, index) => (
            ...
            <>
              {props.id2 === "Religious" ? (
                <>
                  {filteredReligiousArray.map((place, index) => (
                    <div className="card" key={index}>
                      <div className="img">
                        <img src={place.image} alt="Image_of_location" />
                      </div>
                ...
                <>
                  {filteredNaturalArray.map((place, index) => (
                    <div className="card" key={index}>
                      <div className="img">
                        <img src={place.image} alt="Image_of_location" />
                      </div>
